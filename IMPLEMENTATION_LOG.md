...
## Milestone 9.8 — Fed 講話 + 證詞攝取(speeches + testimony)

> **狀態**:✅ 完成(2026-06-13 單一 session)
> **目標**:系統原本只抓 8 次排程 FOMC 聲明,對 Fed 最頻繁的動盤來源——主席證詞(Humphrey-Hawkins)與 Board 演講(Powell/副主席/理事)——完全瞎。把這條補上。
> **為什麼是這一塊**:使用者問「SpaceX 上市、川普說伊朗戰爭結束,美股大漲,系統抓不到」。討論後砍掉 IPO(真能動盤的一年一兩件,且本來就是當天頭條會從新聞流進來)與純地緣政治(無乾淨結構化來源、signal/noise 最差),選 Fed 講話——它是高訊號又**不破壞 M9.6 的乾淨指標**:Fed 官方發布、有權威 `pubDate`,validator 能錨在市場真正得知的時點(跟 FOMC 聲明同款紀律,不像第三方新聞時間戳不可信)。
>
> **TL;DR**:
> - **新 adapter `app/adapters/fed_speeches.py`** — 吃 Board 的兩個 RSS feed(`speeches.xml` + `testimony.xml`),mirror `fomc.py` 的兩階段抓法(defusedxml + httpx + tenacity + body 下載)。`source=EventSource.FOMC`(零 migration,跟 `dot_plot.py` 同款),用 `event_type` 區分 `FED_SPEECH` / `FED_TESTIMONY`。
> - **講者 + 角色抽取** — feed `<title>` 只有姓("Barr, ..."),講者全名與角色從講稿頁的 `<p class="speaker">`("Governor Michael S. Barr")抽,`p.location` 拿場合;粗分 CHAIR / VICE_CHAIR / GOVERNOR 三級存進 payload。
> - **可設定的講者範圍** — `FED_SPEAKER_SCOPE`(`all`(預設) / `principals` / `chair`),env 可調不用 redeploy。
> - **路由** — `FED_TESTIMONY` 加進 `_HIGH_STAKES_EVENT_TYPES`(premium + 3 次多數決,~8/年);`FED_SPEECH` 用預設 gpt-5-mini 單次。新增成本 ~$0.65/年。
> - **零改動就接上** — 新 event_type 不在 `_COMPANY_EVENT_TYPES`,analyzer 自動 MARKET-only(SPY/QQQ);event_writer 靠 `(source, external_id)` 去重;context_builder 自動納入 lookback。只多接一個 task + 一個 beat 排程(twice-daily)。

### 設計決策與真實資料的修正

- **講者範圍 default=all**:Board feed 本身就只含理事會層級(地方分行總裁要另外爬、列為 future work),是個天然的乾淨邊界。env 可收窄。
- **角色從頁面抽,不用硬編名單**:原計畫想「pre-filter 省 body fetch」,但角色只有頁面 `p.speaker` 可靠(title 只有姓)。改成 **fetch-then-filter**——default=all 反正全抓不浪費,窄範圍才丟幾個 GET,換來不用維護「誰是主席」的脆弱對照表。**live smoke 當場驗證了這個決定**:2026-06 Powell 的演講頁標的是「**Governor** Jerome H. Powell」(他主席任期 2026-05 到期、仍是理事)——若硬編「Powell=CHAIR」就會錯,讀頁面才正確反映職務變動。
- **body 容器**:本來怕講稿頁 DOM 跟新聞稿的 `#article` 不同,live 一驗發現**講稿頁也是 `#article`**(27 KB 全文),`div.col-xs-12...` 那個 fallback 反而只撈到 151 字的標頭,直接砍掉,跟 FOMC 完全一致。

### 驗收狀態

- [x] 新 adapter 純函式可單測:講者/角色解析、body 抽取、scope 過濾、兩 feed 合併 — 17 個單元測試
- [x] router 加 2 測(testimony→premium、speech→default);analyzer_v2 加 1 整合測(FED_SPEECH 走 MARKET-only,鎖死「無需改 analyzer」)
- [x] 後端 ruff + format + mypy(strict,71 檔)0 error,154 unit 全綠
- [x] **live smoke 實測**:對真實 feed 抓到 30 筆(15 演講 + 15 證詞),body 全數抓到、講者/角色/pubDate/external_id 都正確(by role:GOVERNOR 14 / VICE_CHAIR 12 / CHAIR 4)
- [ ] 整合測試交給 CI 的 Postgres container(本機 Docker engine 卡死、重啟被權限擋);push 前用 import + `--collect-only` 確認可收集
- [ ] 尚未 push / 部署(等使用者拍板上 prod)
