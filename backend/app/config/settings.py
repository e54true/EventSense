...
class Settings(BaseSettings):
    ...
    # --- v2/v3 contextual analyzer ---
    # Days of recent events the prompt looks back over when analyzing a new
    ...
    # Cap on how many recent events get inlined into the prompt. The window
    # query orders by published_at DESC, so this is "most-recent N" not random.
    analyzer_recent_events_cap: int = Field(default=50)
    # Which prompt template the analyzer should render. Kill switch: flip back
    # via env (ANALYZER_PROMPT_VERSION=v2) if v3 misbehaves in production,
    # no redeploy needed.
    analyzer_prompt_version: Literal["v1", "v2", "v3"] = Field(default="v3")
    # Self-consistency: how many independent LLM calls premium-routed
    # (high-stakes) events get; the analyzer majority-votes per-ticker
    # direction across the samples. 1 = off. Only applies when the router
